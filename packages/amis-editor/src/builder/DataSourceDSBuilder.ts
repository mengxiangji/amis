...
export class DataSourceDSBuilder extends DSBuilder<
  ApiDSBuilderOptions<DSRendererType>
> {
  static key = 'dataSource';

  isDefault = true;

  readonly name: string = '数据源';
  ...
  makeSourceSettingForm(options: ApiDSBuilderOptions<DSRendererType>): any[] {
    ...
    const labelText =
      label ??
      (isCRUD && feat !== 'List'
        ? this.getFeatLabelByKey(feat) + '数据源'
        : '接口');
    ...
    return [
      /** 提交接口 */
      getSchemaTpl('dataSourceControl', baseApiSchemaConfig),
      /** 表单初始化接口 */
  ...
  async buildEditSchema<T extends DSRendererType>(
    ...
    const {editApi, initApi, listApi} = scaffoldConfig || {};
...
registerDSBuilder(DataSourceDSBuilder);
